...
* 暗影模式下噩梦燃料将转变为可食用状态，击杀暗影生物或吃下噩梦燃料可恢复25点理智值上限。除此以外似乎还有回复效果更强的道具存在
* 对暗影生物造成伤害时附加50%的额外伤害
* 免疫火焰伤害，免疫冰冻（不包括过热和过冷伤害）
* 当Musha激活暗影模式时，所属的[影子Musha](#1-影子musha)会进入狂暴状态
* 激活暗影模式需要当前理智值大于等于50点
* 通过执行睡觉指令可以解除暗影模式，并进入20秒冷却时间

#### 1.4.1 暗影模式技能组

* 注：以下技能除[影莲华](#1414-影莲华)外均**可以**在乘骑状态下使用
...
* 默认快捷键：双击R
* 无法直接使用，只有使用[影幻身](#1412-影幻身)后的短时间内可以通过再次按下快捷键施放
