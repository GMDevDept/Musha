...
### 切换四种角色模式
   ...
   4. 女武神模式
      1. 默认快捷键：R
      2. 被攻击时受到的伤害降低20%
      3. 对怪物和敌对生物造成伤害时附加50%的额外伤害
      4. 普通攻击会造成AOE伤害，若武器本身已有AOE攻击模组则AOE范围大幅扩大
      5. 免疫火焰伤害，免疫冰冻（不包括过热和过冷伤害）
      6. 武器不会脱手，落水时不会丢失携带的物品
      7. 通过执行睡觉指令或死亡/重进游戏可以解除女武神模式
   5. 暗影模式
      1. 默认快捷键：G
